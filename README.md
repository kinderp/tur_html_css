## INFORMATICA 2A TUR 24/25: prova pratica al calcolatore (HTML/CSS)
...
Di seguito sono elencati i punti per il completamento del sito, <b>OGNI PUNTO VALE 0.5 PIU' UN PUNTO DI BONUS PER LA PULIZIA E LA QUALITA' DEL CODICE</b>
</p>
...
2. Appena sotto il `<body>`, aggiungi un `div` contenente una lista non ordinata: tag `<ul>`
   * Inserisci tre list items (`<li>`) per le pagine: `home.html`, `courses.html`, `contacts.html` ed aggiungi rispettivamente i seguenti testi: `Home`, `Courses`, `Contacts`
   * Rendi cliccabili (`<a>`) i tre list items (`<li>`) in modo da reindirizzare l'utente alla pagina corretta
